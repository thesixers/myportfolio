...
export default function ProjectCard({ project }) {
  const { name, type, preview, description, technologies, repo, link } = project;
  ...
  const mainButtonText = type === "web" ? "Visit" : "Download";
    ...
    >
      ...
      <div className="relative h-48 overflow-hidden bg-slate-100 dark:bg-slate-800 border-b border-slate-100 dark:border-white/5">
        ...
        <div className="absolute top-3 right-3 z-10 bg-white/90 dark:bg-slate-900/90 backdrop-blur-md px-3 py-1 rounded-full text-xs font-bold text-slate-700 dark:text-slate-300 shadow-sm flex items-center gap-1 border border-transparent dark:border-white/10">
          {type === "mobile" ? <span>📱 App</span> : <span>🌐 {type}</span>}
        </div>
        ...
        {/* Overlay for Desktop */}
        <div className="absolute inset-0 bg-indigo-900/60 dark:bg-slate-950/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-3 backdrop-blur-[2px] hidden md:flex">
          {link && (
            <a
              href={link}
              target="_blank"
