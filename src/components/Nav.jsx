...
// 1. The Dynamic Logo Component (Same as before)
...
export default function Nav({ currentNav, setCurrentNav }) {
    ...
    return (
        <div className="fixed bottom-6 inset-x-0 flex justify-center z-50 pointer-events-none">
            <nav className="flex items-center gap-1 p-2 bg-white/80 backdrop-blur-xl border border-white/40 rounded-full shadow-2xl pointer-events-auto overflow-hidden">
                {navItems.map((item) => {
                    ...
                    return (
                        <a 
                            ...
                            className="group relative px-3 md:px-6 py-3 md:py-2 rounded-full transition-colors duration-300 flex items-center justify-center"
                        >
                            <span className={`relative z-10 ${isActive ? 'text-white' : 'text-slate-500 hover:text-indigo-500'}`}>
                                {item.isLogo ? (
