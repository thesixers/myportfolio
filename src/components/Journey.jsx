...
export default function Journey() {
  ...
  return (
    <section ref={ref} className="py-24 max-w-4xl mx-auto relative px-4 overflow-hidden">
      ...
      {/* The Central Line - Darker in Dark Mode */}
      <div className="absolute left-8 md:left-1/2 top-40 bottom-20 w-1 bg-slate-100 dark:bg-slate-800 origin-top transform -translate-x-1/2">
        <motion.div 
            style={{ scaleY }} 
...
const EventCard = ({ event, index }) => {
    ...
    return (
        ...
        >
            {/* The Dot on the Line - White border vs Dark border */}
            <div className="absolute left-8 md:left-1/2 transform -translate-x-1/2 w-5 h-5 bg-white dark:bg-slate-900 border-4 border-indigo-500 rounded-full z-10 shadow-[0_0_0_4px_rgba(99,102,241,0.2)] dark:shadow-none"></div>
            ...
            <div className={`pl-16 md:pl-0 w-full md:w-1/2 ${isEven ? 'md:text-right md:pr-12' : 'md:pl-12'}`}>
                ...
                ">
                    <span className="inline-block bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 px-3 py-1 text-xs font-bold rounded-full mb-2">
                        {event.year}
                    </span>
                    <h3 className="text-xl font-bold text-slate-900 dark:text-white">{event.title}</h3>
                    <p className="text-slate-500 dark:text-slate-400 text-sm mt-2 leading-relaxed">{event.desc}</p>
                </div>
            </div>
        </motion.div>
    )
